...
import { getF2Data } from '@/lib/laporan/queries'
...
export default function LaporanF2Page() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await getF2Data(params.id as string, bulan, tahun)
        setData(res)
      } catch (error) {
        console.error(error)
        toast.error('Gagal mengambil data F2 Kelahiran/Kematian RT')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      await generateAndDownloadPDF(
        F2KelahiranRT,
        { data },
        `F2_Kelahiran_RT_${data.info?.nomor_rt || params.id}_${bulan}_${tahun}`
      )
      toast.success('Laporan F2 berhasil diunduh')
    } catch (error) {
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Preview Laporan F2</h1>
            <p className="text-muted-foreground">Rekapitulasi Data Kelahiran & Kematian Tingkat RT (Landscape)</p>
          </div>
        </div>
        <Button onClick={handleDownload} disabled={generating} className="h-10 px-6">
          {generating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
          {generating ? 'Memproses Data...' : 'Unduh Laporan F2'}
        </Button>
      </div>
      ...
      <Card className="border-none shadow-sm overflow-hidden">
        <CardHeader className="border-b bg-muted/30">
          <div className="flex items-center gap-2 text-primary">
            ...
            <CardTitle className="text-lg">Pratinjau Tabel Kelahiran/Kematian F2</CardTitle>
          </div>
        </CardHeader>
          ...
          <div className="p-4 overflow-auto bg-slate-100">
             <div className="min-w-[1200px] bg-white p-8 shadow-sm">
                <div className="text-center mb-6">
                   ...
                   <p className="uppercase text-sm">RT: {data.info?.nomor_rt || '-'} | BULAN: {bulanNames[bulan]?.toUpperCase()} {tahun}</p>
                </div>
                   ...
                   <table className="w-full border-collapse border border-black text-center">
                      <thead>
                         <tr className="bg-muted/50 font-bold">
                            ...
                            <th rowSpan={2} className="border border-black p-2 align-middle">NAMA KELOMPOK DASAWISMA</th>
                      ...
                      <tbody>
                         {items.length === 0 ? (
                            <tr>
                               <td colSpan={15} className="border border-black p-4 text-center text-muted-foreground">Tidak ada kelompok Dasawisma tercatat.</td>
                            </tr>
                         ) : (
                            <>
                              {items.map((row: any, i: number) => (
                                <tr key={i} className="hover:bg-muted/20">
                                   ...
                                   <td className="border border-black p-1.5 text-left uppercase font-medium">{row.nama_dasawisma}</td>
