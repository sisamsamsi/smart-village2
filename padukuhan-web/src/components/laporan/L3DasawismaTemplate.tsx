    ...
    <Document>
      <Page size="FOLIO" orientation="landscape" style={styles.pageLandscape}>
        ...
        <View style={styles.judulContainer}>
          <Text style={styles.judul}>REKAPITULASI CATATAN DATA WARGA KELOMPOK DASA WISMA</Text>
          <Text style={styles.subtitle}>DASA WISMA: {info?.nama_dasawisma?.toUpperCase() || '-'} | RT: {info?.rts?.nomor_rt || '-'}</Text>
        </View>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.tableRow}>
            ...
            <View style={[styles.tableColHeader, { width: 80 }]}><Text style={styles.tableCellHeader}>NAMA KEPALA RUMAH TANGGA</Text></View>
          ...
          {items.map((row: any, idx: number) => (
            <View key={idx} style={styles.tableRow}>
              ...
              <View style={[styles.tableCol, { width: 80 }]}><Text style={styles.tableCell}>{row.nama_krt?.toUpperCase()}</Text></View>
              ...
              <View style={[styles.tableCol, { width: 35 }]}><Text style={styles.tableCellCenter}>{row.sehat_layak ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 35 }]}><Text style={styles.tableCellCenter}>{row.tidak_sehat ? 'V' : ''}</Text></View>
              
              <View style={[styles.tableCol, { width: 35 }]}><Text style={styles.tableCellCenter}>{row.ada_tempat_sampah ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 30 }]}><Text style={styles.tableCellCenter}>{row.ada_spal ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 30 }]}><Text style={styles.tableCellCenter}>{row.ada_jamban ? 'V' : ''}</Text></View>
              
              <View style={[styles.tableCol, { width: 30 }]}><Text style={styles.tableCellCenter}>{row.stiker_p4k ? 'V' : ''}</Text></View>
              
              <View style={[styles.tableCol, { width: 25 }]}><Text style={styles.tableCellCenter}>{row.sumber_pdam ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 25 }]}><Text style={styles.tableCellCenter}>{row.sumber_sumur ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 25 }]}><Text style={styles.tableCellCenter}>{row.sumber_dll ? 'V' : ''}</Text></View>
              
              <View style={[styles.tableCol, { width: 25 }]}><Text style={styles.tableCellCenter}>{row.beras ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 25 }]}><Text style={styles.tableCellCenter}>{row.non_beras ? 'V' : ''}</Text></View>
              
              <View style={[styles.tableCol, { width: 25 }]}><Text style={styles.tableCellCenter}>{row.up2k ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 35 }]}><Text style={styles.tableCellCenter}>{row.pmt_pekarangan ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 35 }]}><Text style={styles.tableCellCenter}>{row.industri_rt ? 'V' : ''}</Text></View>
              <View style={[styles.tableCol, { width: 30 }]}><Text style={styles.tableCellCenter}>{row.kerja_bakti ? 'V' : ''}</Text></View>
